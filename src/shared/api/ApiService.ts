...
export class BaseRestClient {
    ...
    protected prepareToken(token: string = ""): { headers: { Authorization: string } } {
        return {
            headers: {
    ...
    protected async download(url: string, config?: AxiosRequestConfig): Promise<Blob> {
        const response: AxiosResponse<Blob> = await this.axiosInstance.request<Blob>({
            url,
            method: "GET",
            responseType: "blob",
            ...config,
    ...
    private async request<T>(
        ...
        try {
            const response: AxiosResponse<BaseResponse<T>> = await this.axiosInstance.request({
                url,
                method,
                data,
                ...config,
    ...
    private handleError(error: unknown): never {
        if (axios.isAxiosError(error)) {
            const message = error.response
                ...
                : "Network Error";
            throw new Error(error?.response?.data.message || `${message} - ${error.message}`);
        }
